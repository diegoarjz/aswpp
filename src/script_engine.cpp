...
static void StaticMessageCallback(const asSMessageInfo *msg) {
  ...
  case asMSGTYPE_ERROR:
    std::cerr << "[Error] ("<< msg->section << "@" << msg->row << ":" << msg->col << ")" << msg->message << std::endl;
    break;
  case asMSGTYPE_WARNING:
    std::cerr << "[Warning] ("<< msg->section << "@" << msg->row << ":" << msg->col << ")" << msg->message << std::endl;
    break;
  case asMSGTYPE_INFORMATION:
    std::cerr << "[Info] ("<< msg->section << "@" << msg->row << ":" << msg->col << ")" << msg->message << std::endl;
    break;
  };
}
...
public:
  Impl(Engine& engine)
    ...
    m_asEngine->SetMessageCallback(asFUNCTION(StaticMessageCallback), nullptr, asCALL_CDECL);
...
Engine::Engine() 
  : m_impl(std::make_unique<Engine::Impl>(*this)) {
}
...
bool Engine::IsValid() const {
  return m_impl->m_asEngine != nullptr;
}

ModulePtr Engine::CreateModule(const std::string& name, const std::string& source)
{
...
bool Engine::Attach(const ModulePtr& module) {
  ...
  const bool wasInserted = m_impl->m_modules.emplace(module->GetName(), module).second;
  if (!wasInserted) {
    return false;
  }
  module->attachTo(this);

  if (!module->prepareModule(reinterpret_cast<asScriptEngineHandle*>(m_impl->m_asEngine))) {
    return false;
  }
  ...
  void reportSetFunctionError(int error) {
    if (error != asSUCCESS) {
      if (error == asCONTEXT_NOT_PREPARED) {
        ...
        std::cerr << " Argument number is larger than the number of arguments in the prepared function." << std::endl;
      }
...
template<>
bool Engine::setFunctionArg<int64_t>(int i, int64_t val) {
  const int r = m_impl->m_context->SetArgQWord(i, val);
  if (r != asSUCCESS) {
    std::cerr << "Unable to set argument " << i << " to value at address " << val << std::endl;
...
template<>
bool Engine::setFunctionArg<uint64_t>(int i, uint64_t val) {
  const int r = m_impl->m_context->SetArgQWord(i, val);
  if (r != asSUCCESS) {
    std::cerr << "Unable to set argument " << i << " to value at address " << val << std::endl;
    reportSetFunctionError(r);
...
template<>
bool Engine::setFunctionArg<int32_t>(int i, int32_t val) {
...
template<>
bool Engine::setFunctionArg<uint32_t>(int i, uint32_t val) {
  const int r = m_impl->m_context->SetArgDWord(i, val);
  if (r != asSUCCESS) {
    std::cerr << "Unable to set argument " << i << " to value at address " << val << std::endl;
    reportSetFunctionError(r);
...
template<>
bool Engine::setFunctionArg<int16_t>(int i, int16_t val) {
...
template<>
bool Engine::setFunctionArg<uint16_t>(int i, uint16_t val) {
  const int r = m_impl->m_context->SetArgWord(i, val);
  if (r != asSUCCESS) {
    std::cerr << "Unable to set argument " << i << " to value at address " << val << std::endl;
    reportSetFunctionError(r);
...
template<>
bool Engine::setFunctionArg<int8_t>(int i, int8_t val) {
...
template<>
bool Engine::setFunctionArg<uint8_t>(int i, uint8_t val) {
  const int r = m_impl->m_context->SetArgByte(i, val);
  if (r != asSUCCESS) {
    std::cerr << "Unable to set argument " << i << " to value at address " << val << std::endl;
    reportSetFunctionError(r);
...
template<>
bool Engine::setFunctionArg<char>(int i, char val) {
  const int r = m_impl->m_context->SetArgByte(i, val);
  if (r != asSUCCESS) {
    std::cerr << "Unable to set argument " << i << " to value at address " << val << std::endl;
    reportSetFunctionError(r);
...
template<>
bool Engine::setFunctionArg<float>(int i, float val) {
...
template<>
bool Engine::setFunctionArg<double>(int i, double val) {
