...
public:
  ...
  bool PrepareModule(asIScriptEngine* engine) {
    ...
    if (builder.AddSectionFromMemory("section", m_source.c_str(), m_source.size(),
                                     0) != 1) {
...
bool Module::IsAttached() const {
  return m_impl->m_attachedTo != nullptr;
}

void Module::attachTo(Engine* engine) {
  m_impl->m_attachedTo = engine;
}
